<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if (!isset($jsonData->refresh_token) || strlen($jsonData->refresh_token) < 1) {
      ...
      $response->setHttpStatusCode(400);
      $response->setSuccess(false);
      ((empty($jsonData->refresh_token) || !isset($jsonData->refresh_token)) ? $response->addMessage("Refresh token not supplied") : false);
      ((empty($jsonData->refresh_token) || strlen($jsonData->refresh_token) < 1) ? $response->addMessage("Refresh token cannot be blank") : false);
      $response->send();
      exit;
    }
    ...
    try {
      ...
      from tblsessions, tblusers where tblusers.id = tblsessions.userid and tblsessions.id = :sessionid and 
      tblsessions.accesstoken = :accesstoken and tblsessions.refreshtoken = :refreshtoken');
      ...
      refreshtokenexpiry = date_add(NOW(), INTERVAL :refreshtokenexpiryseconds SECOND) where 
      id = :sessionid and userid = :userid and accesstoken = :returnedaccesstoken and refreshtoken = :returnedrefreshtoken');
      $query->bindParam(":sessionid", $sessionid, PDO::PARAM_INT);
